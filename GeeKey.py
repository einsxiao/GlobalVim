...
class GeeKeyFrame(wx.Frame):
    def __init__(self,parent,ID):
        ...
        self.data_dir = os.path.join(self.user_dir,'AppData','Roaming','GlobalVim')
    ...
    def makePanel(self):
        ...
        self.makeGeeKeyButton('u',(530+bx,170+by),keytype='macro')
        self.makeGeeKeyButton('i',(593+bx,170+by),keytype='macro')
        self.makeGeeKeyButton('o',(656+bx,170+by),keytype='macro')
        ...
        self.makeGeeKeyButton("f",(355+bx,231+by),keytype='edit')
        ...
        self.makeGeeKeyButton("b",(415+35+bx,292+by),keytype='edit' )
        self.makeGeeKeyButton("n",(478+35+bx,292+by),keytype='edit' )
        self.makeGeeKeyButton("m",(541+35+bx,292+by),keytype='macro' )
        self.makeGeeKeyButton(",",(604+35+bx,292+by),keytype='macro' )
        self.makeGeeKeyButton(".",(667+35+bx,292+by),keytype='macro' )
    ...
    def LoadConfig(self,filename='default.ini',changeMenuKey=False):
        ...
        self.config[ 'version' ] = INFO['Version']
        ### configfile settings 
    ...
    def makeGeeKeyButton(self,name,pos,size=KeySize,keytype='plain'):
        ...
        if keytype in ('macro','text','edit','function'): ### geekey keys
            ...
            ##########################################
            self.key_combos[name] = wx.ComboBox( self.keyboardPanel,value='',pos=cpos,size=(max(100,size[0]),20),choices = EditKeys ) 
    ...
    def _Replay(self,ratio,sequence):
        ...
        if len( oper ) <= 2: #compact oper for keyboard  :_x~0.1  :x~  :x  :_x
            ...
            KeySend( key = key, keytype = keytype, scancode = ScanCodeReplay )
    ...
    def OnKeyboardEvent(self, Key, EvtType, EvtScanCode ): 
        try:
            IsKeyDown = EvtType == 'key down' or EvtType == 'key sys down'
            IsKeyUp = EvtType == 'key up' or EvtType == 'key sys up'
            ...
            if self.state_is_recording and not EvtScanCode == ScanCodeRevised and not EvtScanCode == ScanCodeReplay:
            ...
            if  not Key in self.GeeKeyKeys:
                ...
                if Key == 'left shift' or Key == 'right shift':
                    ...
                    #print('left shift set',self.state_on_shift)
                    return True
                ###################################################
                #alt key state
            ...
            if EvtScanCode in (ScanCodeRevised,ScanCodeFinal): ### normal keys
