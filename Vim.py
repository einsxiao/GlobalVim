...
class VimStateIndicator(wx.Frame):
    def __init__(self,parent,mainFrame,vim):
        ...
        ###
        font = wx.Font(11, wx.ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_LIGHT,)
    ...
    def GetPosition(self):
        ...
        try:
            ...
            rect = self.sys_rect 
            pass
        if rect[3] > self.sys_rect[3]: pos = ( int(rect[2]/2+rect[0]/2), self.sys_rect[3] +15 - self.size[1] )
        else: pos  = ( int(rect[2]/2+rect[0]/2), rect[3] + 15 - self.size[1] )
        self.last_pos = pos
        return pos
...
class Vim:
    ...
    def ProcessKey(self,Key,EvtType):
        #print('processkey',EvtType,Key,self.geekey.state_on_shift)
        # esc not included
        ...
        if oper in ('left','right','up','down','return','home','end','page up','page down'):
        ...
        ############################################################ 
            ...
            KeyPress( 'left ctrl' )
            for i in range(oper_number() ): KeyStroke( 'right' )
            KeyRelease( 'left ctrl' )
        elif oper == 'end word':
            if not self.visual_mode: KeyPress( 'left shift' )
            ...
            KeyPress( 'left ctrl' )
            for i in range(oper_number() ): KeyStroke( 'left' )
            KeyRelease( 'left ctrl')

        elif oper == 'prev para':
            KeyPress( 'left ctrl' )
            for i in range(oper_number() ): KeyStroke( 'up' )
            KeyRelease( 'left ctrl' )

        elif oper == 'next para':
            KeyPress( 'left ctrl' )
            for i in range(oper_number() ): KeyStroke( 'down' )
            KeyRelease( 'left ctrl' )
            ...
            if not self.visual_mode:
                KeyPress( ('left shift','left ctrl',) )
                for i in range(oper_number() ): KeyStroke('right')
                KeyRelease( ('left ctrl','left shift') )
            ...
            #log.log('visual mode')
            if not self.visual_mode:
                if oper == 'line visual mode': KeyStroke('home')
                KeyPress('left shift',ScanCodeFinal)
        ...
        # paste part
            ...
            if self.getRegister():
                if self.line_cut: # paste to a single line
                    ...
                    KeyRelease('left ctrl')
                pass
        elif oper == 'undo' : KeyPress('left ctrl'); KeyStroke('z'); KeyRelease('left ctrl')
        elif oper == 'redo' : KeyPress('left ctrl'); KeyStroke('y'); KeyRelease('left ctrl')
        elif oper == 'merge line': KeyStroke('end'); KeyStroke('delete')
        elif oper == 'find' :
            if self.visual_mode: # the same as d cut
                ...
                KeyPress('left ctrl'); KeyStroke('f'); KeyRelease('left ctrl')
                self.insert_mode = True
                self.indicator.StateReset()
                pass
            pass
            ...
            if self.visual_mode: # the same as d cut
                ...
                KeyPress('left shift');  KeyPress('left ctrl')
                KeyStroke('right'); KeyRelease('left shift')
                KeyStroke( ('c','f','v') ); KeyRelease('left ctrl')
