...
import image_bridge
#import image_wechat_code
#import image_wechat_code_alt

#from contextlib import redirect_stdout, redirect_stderr
#log_file = os.path.join( os.path.dirname( __file__ ), 'vg.log')
#log_out = open(log_file,'a') 
#sys.stdout = log_out
#sys.stderr = log_out
#redirect_stdout(log_out)
#redirect_stderr(log_out)
...
def warning(message,title=''):
    ...
    dlg = wx.MessageDialog(global_app,message,title,style = style)
    dlg.ShowModal()
    dlg.Destroy()
...
class GeeKeyBoard:
    ...
    def coverKey(self,key):
        if key in ('shift','alt','ctrl','windows'):
            lkey = "left "+key
            rkey = 'right '+key
            if keyboard.is_pressed(lkey):
                geeKeyboard.keyRelease( lkey )
            if keyboard.is_pressed(rkey):
                geeKeyboard.keyRelease( rkey )
            pass
        else:
            if keyboard.is_pressed(key): geeKeyboard.keyRelease( key )

    def recoverKey(self,key):
        if key in ('shift','alt','ctrl','windows'):
            lkey = "left "+key
            if keyboard.is_pressed(lkey):
                geeKeyboard.keyPress( lkey )
            rkey = 'right '+key
            if keyboard.is_pressed(rkey):
                geeKeyboard.keyPress( rkey )
            pass
        else:
            #log.log(key,'recovered')
            if keyboard.is_pressed(key): geeKeyboard.keyPress( key )
        
    def keyPress(self,key,scancode=ScanCode_revised):
        user32.keybd_event(KeyMap[ key ], scancode, self.KEYDOWN, 0) 
        pass

    def keyRelease(self,key,scancode=ScanCode_revised):
        user32.keybd_event(KeyMap[ key ], scancode, self.KEYUP, 0)
        pass

    def keyStroke(self,key,scancode=ScanCode_revised):
    ...
    def keySend(self,key,keytype = 'key down',repeated = 1,scancode = ScanCode_revised):
        try:
...
StringKeys = NumberKeys + CharKeys + SpecKeys

MenuKeys_1 = [ 'tab','caps lock','left shift','left ctrl',
             'left windows','left alt','right alt',
             'right windows','menu','right ctrl','right shift' ]
...
GlobalMaps['macro'] = {
    "t":"",
    "y":"copy:0:_right ctrl:_c:c:right ctrl",
    "p":"paste:0:_right ctrl:_v:v:right ctrl",
    "f2":"save:0:_right ctrl:_s:s:right ctrl",
    "w":"word next:0:_right ctrl:_right:right:right ctrl",
    "b":"word back:0:_right ctrl:_left:left:right ctrl",
    "":"",
}
GlobalMaps['edit'] = {
...
KeyStroke = lambda Key: geeKeyboard.keyStroke(Key)
KeyPress = lambda Key: geeKeyboard.keyPress(Key)
KeyRelease = lambda Key: geeKeyboard.keyRelease(Key)
KeySend = lambda key,keytype='key down',repeated=1,scancode=ScanCode_revised: geeKeyboard.keySend(key,keytype,repeated,scancode )
SetKeyDelay = lambda delay: geeKeyboard.setKeyEventDelay(delay)
TextSend = lambda txt: geeKeyboard.textSend( txt )
LetterSend = lambda letter: os_keyboard.type_unicode(letter)
...
def WxCallLater(delay,func,*args,**kwargs):
    return wx.CallLater(delay*1000, func, *args, **kwargs)
